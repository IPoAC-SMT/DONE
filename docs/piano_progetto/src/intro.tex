...
\begin{center}
    \begin{tikzpicture}
        \node[rectangle,draw,minimum height=4cm,minimum width=2.5cm,fill=gray!50] (net) at (-5,0) {Networking};
        \node[rectangle,draw,minimum height=4cm,minimum width=2.5cm,align=center,fill=gray!35] (log) at (-1,0) {Logical\\Controller};
        \node[rectangle,draw,minimum height=1.5cm, minimum width=2.5cm,fill=gray!15] (gui) at (3,1.25) {GUI};
        \node[rectangle,draw,minimum height=1.5cm, minimum width=2.5cm,fill=gray!15] (cli) at (3,-1.25) {CLI};
        \draw[<->, shorten >= 5pt, shorten <= 5pt] (net) -- (log);
        \draw[<->, shorten >= 5pt, shorten <= 5pt] (log) -- (gui);
        \draw[<->, shorten >= 5pt, shorten <= 5pt] (log) -- (cli);
    \end{tikzpicture}
\end{center}
